...
finisher = 'It was really nice talking to you and I hope that now you feel better after talking to me. Best of luck for your future endeavours. Bye!'
...
def friends():
    ...
    if(predict(response) >=0.4):
        ...
        if(predict(response)>=0.4):
            print(name + ", Don't feel sad. Take your time and heal properly,look at what's happened, learn from it, and find ways to build a new and healthy life.All any of us wants is to be happy. For some, this requires the perfect person to be our other half, and for others, it means completing the equation yourself. Either way, to find the right person, you need to be the right person. And trust that in the long run, your efforts will lead to your own personal happy ending.")
            print(finisher)
        else:
            print(name + ", Don't worry. You may be at a point where similar people are not in your life right now. That happens in life from time to time.It is better to be away from incompatible people, and those people are attracted to you when you pretend to be someone you aren't. Be as different as you truly are, get to know yourself at a deep level, esteem your individuality, interact with pepole honestly, and eventually the people who appreciate you will notice and be drawn in.")
            print(finisher)
    else:
        print("Many people tend to expect too much of others, their family, their friends or even just acquaintances. It's a usual mistake, people don't think exactly the way you do.\nDon't let the opinions of others make you forget what you deserve. You are not in this world to live up to the expectations of others, nor should you feel that others are here to live up to yours. The first step you should take if you want to learn how to stop expecting too much from people is to simply realize and accept the fact that nobody is perfect and that everyone makes mistakes every now and then.")
        print(finisher)

def family():
    print(name + ", Don't take too much stress. All you need to do is adjust your priorities. Don't take on unnecessary duties and responsibilities.Take advice from people whose opinion you trust, and get specific advice when issues arise.\nYou should use stress management techniques and always hope for the best. These situations arise in everyone's life and what matters the most is taking the right decision at such moments.")
    print(finisher)

def work():
    print(name + ", Don't take too much stress. I can list some really cool ways to handle it.You should develop healthy responses which include doing regular exercise and taking good quality sleep. You should have clear boundaries between your work or academic life and home life so you make sure that you don't mix them.Tecniques such as meditation and deep breathing exercises can be really helping in relieving stress. Always take time to recharge so as to avoid the negative effects of chronic stress and burnout. We need time to replenish and return to our pre-stress level of functioning.")
    print(finisher)

def sad1():
    response = input('I understand. Seems like something\'s bothering you. Could you further describe it, in short?')
    if(predict(response)>=0.4):
        response = input('It seems like though the issue might be a little worrisome, it might not actually be very serious. What are your thoughts on this?')
        if(predict(response)>=0.5):
            response = input('Looks like you agree with me. Wanna sign off?')
            if(predict(response)>0.55):
                print("That's okay. It was nice talking to you. You can chat with me anytime you want.Bye!")
            else:
                sad3()
        else:
            sad3()
...
def sad2():
    response = input('Please feel free to share your feelings, think of me as your friend.')
    if(predict(response)>=0.3):
        ...
        print("Okay. You just identified what we call an automatic thought. Everyone has them. They are thoughts that immediately pop to mind without any effort on your part.Most of the time the thought occurs so quickly you don't notice it, but it has an impact on your emotions. It's usually the emotion that you notice, rather than the thought.\nOften these automatic thoughts are distorted in some way but we usually don't stop to question the validity of the thought. But today, that's what we are going to do.")
        response = input('So, are there signs that contrary could be true?')
        if(predict(response)>=0.4):
            print("I'm glad that you realised that the opposite could be true. The reason these are called 'false beliefs' is because they are extreme ways of perceiving the world. They are black or white and ignore the shades of grey in between.Now that you have learned about this cool technique, you can apply it on most of the problems that you will face. If you still feel stuck at any point, you can always chat with me.Best of luck for your future endeavours. Bye!")
        else:
            sad4()
    else:
        sad4()
...
if (predict(response) >= 0.55):
    response = input('That is good. Are you usually this happy, or are there some worries that you want to talk about?')
    if (predict(response)>=0.7):
        response = input('You seem to be really content. Wanna sign off?')
        if(predict(response)>=0.7):
            print('Ok, bye!')
        else:
            response = input('Is there something bothering you? Would you share it with me?')
            if(predict(response)>=0.7):
                print("That's okay. It was nice talking to you. You can chat with me anytime you want. Bye!")
            else:
                sad1()
